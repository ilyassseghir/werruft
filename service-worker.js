...
/**
 * Kontaktformular Nachricht speichern
 * Erwartet eine Tabelle `contact_messages` mit Spalten:
 *   name (text), email (text), subject (text), message (text), created_at (timestamp, default now())
 */
async function sendContactMessageToDB({ name, email, subject = '', message }) {
  try {
    const { error } = await supabaseClient
    ...
    console.error('❌ Fehler beim Kontaktformular:', e);
    return false;
  }
}
...
window.DB = {
    ...
    sendContactMessage: sendContactMessageToDB,
